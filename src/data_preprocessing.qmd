...
This document contains the code for data preprocessing. The data preprocessing is done to clean the data and prepare it for analysis. The purpose is to reduce rendering time in `main.qmd` each time the user renders the file.

The raw dataset (`"data/gistemp1200_GHCNv4_ERSSTv5.nc"`) is read in using `ncdf4` package. The data is then cleaned by removing missing values and interpolating them. The data is then reshaped into a long format dataframe. The data is then filtered for years 1960 to 2024. The mean temperature anomaly for each year is calculated. The data is then exported to a csv file for visualization in `main.qmd`.
