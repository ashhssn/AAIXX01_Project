...
packages <- c("ncdf4", "reshape2", "plotly", "dplyr", "rnaturalearth", "zoo", "scales", "data.table", "sf")
...
Before cleaning our dataset, we need to first perform EDA to understand the structure of our data. We will load the data, slice it to the past 6 decades, and check for missing values. We will also visualize the missing values to understand the pattern of missingness.
...
anomaly <- ifelse(anomaly == fill_value, NA, anomaly) * (scale_factor * 100)
...
For this project, we are only interested in data for the past 6 decades.
...
### 1.1.5 Pattern of missing values {#pattern-of-missing-values}
...
ggplot(missing_data, aes(y = year_month_date, x = 1, fill = missing_count)) +
  ...
  labs(
    ...
    y = "Year-Month",
    fill = "Missing Values",
    caption = "Data Source: https://data.giss.nasa.gov/pub/gistemp/README.txt"
...
To understand the trend of temperature anomaly over time, we plot a time series graph of the global average temperature anomaly by month. This will help us visualize the changes in temperature anomaly over the past 6 decades.
...
ggplot(yearly_data, aes(x = year, y = mean_temp_anomaly)) +
  ...
  labs(title = "Global Average Temperature Anomaly Over Years", x = "Time", y = "Global Average Temperature Anomaly", caption = "Data Source: https://data.giss.nasa.gov/pub/gistemp/README.txt") +
  theme_minimal() +
  theme(
    plot.caption = element_text(hjust = 0.5)
  )
...
### 1.2.4 Categorize geographical zones

```{r}
#| warning: false
#| echo: false
#| label: categorize-geographical-zones


df_clean <- df_clean %>%
  mutate(zone = mapply(categorize_location, lat, lon))
...
df_with_country$mean_anomaly <- round(df_clean$mean_anomaly, 2)

max <- max(df_with_country$mean_anomaly)
min <- min(df_with_country$mean_anomaly)
...
positions <- c(0,0.0625, 0.125, 0.1875, 1)
...
fig <- 
  ...
  add_markers(
    text = ~ paste("Longtitute: ",lon," Latitute: ", lat, "<br />", "Mean Anomaly: ", mean_anomaly, "°C", "<br />","Country: ", name, "<br />", "Zone: ", zone),
    symbol = I("square"),
    size = I(8),
    alpha = 0.4,
    color = ~mean_anomaly,
  ...
  colorbar(
    ...
    limits = c(min, max), 
    tickvals = seq(min, max, 1), 
    ticktext = sprintf("%.1f", seq(min, max, by = 1)),
    len = 0.9,
    orientation = "h", 
    x = 0.41, 
    y = -0.05, 
  ...
  geo = g,
  title = list(text="Global Temperature Anomaly (with respect to 1950 - 1980 base period)",
  ...
  annotations = list(
    list(
      x = 0.5,
      y = -0.4,
      text = '<a href="https://data.giss.nasa.gov/pub/gistemp/README.txt" target="_blank">Data Source</a>',
      showarrow = FALSE,
      xref = 'paper',
      yref = 'paper',
      xanchor = 'center',
  ...
  margin = list(b = 150)  # Adjust bottom margin for the annotation
)
