...
This section presents the original visualization that we will improve upon with data pre-processing. The aim is to enhance the data presentation's clarity, accuracy, and overall effectiveness.

![Original Visualization](main_files/mediabag/critique.png)
...
fig <- plot_geo(df_with_country, lat = ~lat, lon = ~lon, frame = ~year)
...
fig <- fig %>% layout(
  ...
  geo = g,
  title = "Global Temperature Anomaly (with respect to 1950 - 1980 base period)",
  geo = g,
  annotations = list(
    list(
      x = 0.5,
