...
df_sf <- st_as_sf(df_subset, coords = c("lon", "lat"), crs = 4326)
...
# Create the plot
plot <- plot_ly(df_with_country, type = "scattergeo", mode = "markers",
                lon = ~lon, lat = ~lat,
                marker = list(size = 10,
                              symbol = "square",
                              opacity = 0.1,
                              ...
                              colorscale = "RdBu",
                              colorbar = list(title = "Temperature Anomaly (C)")),
                text = ~paste("Country: ", name, "<br>Longitude: ", lon, "<br>Latitude: ", lat,
                              "<br>Temperature Anomaly: ", anomaly)) %>%
  layout(
    title = paste("Temperature Anomaly on", date_to_plot),
    geo = list(
      projection = list(type = "natural earth"),
...
plot <- plot_ly(df_with_country, type = "scattergeo",
                lon = ~lon, lat = ~lat,
                marker = list(
                  size = 10,
                  symbol = "circle",
