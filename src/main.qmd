...
A linear interpolation is performed to fill in the missing values in the `anomaly` column. We chose the linear interpolation method as it is a simple and effective way to estimate missing values in a time series dataset.
```{r}
#| warning: false
#| echo: true
...
We create a new column, `mean_anomaly`, to store the mean temperature anomaly for each year. This will help us to visualize the trend of temperature anomaly over the years.
...
Here, we sample the interpolated `dataframe` again to **speed up** the process of plotting the line graph of before interpolation and after interpolation. This is to see if our interpolation was successful and does not introduce any new patterns.
...
### 1.2.4 Spatial Join with Countries Data
...
# Load the countries data from rnaturalearth
...
df <- setDT(df_with_country)

# Rename the country column to something more descriptive
setnames(df, "name", "country")

# Apply the categorize_location function to create a new column
df[, geographical_zone := mapply(categorize_location, lat, lon)]

# Ensure the data.table has the necessary columns for visualization
df_clean <- df[, .(year, lon, lat, mean_anomaly, country, geographical_zone)]
...
### 2.1.1 Interactive Choropleth Map

```{r}
```

### 2.1.2 Improved Interactive Chloropleth Map with Geographical Information

```{r}
#| warning: false
#| echo: false
#| label: interactive-choropleth-map-improved


# Define color ramp for the plot
col3 <- colorRamp(c("#0C7BDC", "white", "#FFC20A"))

# Define the map projection and layout settings
g <- list(
...
# Round mean anomaly to 2 decimal places
df_clean$mean_anomaly <- round(df_clean$mean_anomaly, 2)

# Create the plotly figure
fig <- plot_geo(df_clean, lat = ~lat, lon = ~lon, frame = ~year)

# Add markers to the plot with hover information
fig <- fig %>%
  add_markers(
    text = ~ paste("Longitude: ", lon, "<br>",
                   "Latitude: ", lat, "<br>",
                   "Country: ", country, "<br>",
                   "Zone: ", geographical_zone, "<br>",
                   "Mean Anomaly: ", format(mean_anomaly, digits = 2), "°C"),
    hoverinfo = "text",
    symbol = I("square"),
    size = I(8),
    alpha = 0.4,
